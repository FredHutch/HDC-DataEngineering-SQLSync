...
exec dbo.DiffTable @TargetDatabaseName='uw_rep_EDWv3_dev', @TargetTableName='UnitTest_TargetTable2', @Debug=0
...
select TotalRowcount = count(*)
    ...
    ,EndedOriginalData = sum(case when NvarcharColumn like 'TestData%' and HDCPEndDate < '9999-12-31' then 1 else 0 end)
    ,UpdatedData = sum(case when NvarcharColumn like 'UpdatedTestData%' then 1 else 0 end)
    ,ActiveUpdatedData = sum(case when NvarcharColumn like 'UpdatedTestData%' and HDCPEndDate = '9999-12-31' then 1 else 0 end)
